...
procedure Word_Ladder is
   ...
   package B_Str is new Ada.Strings.Bounded.Generic_Bounded_Length (Max => 12);
   use B_Str;
   type Word_Rec_T is record
      Parent, Word : Bounded_String;
   end record;

   package Word_Vectors is new Ada.Containers.Vectors (Positive, Bounded_String);
   ...
   begin
      ...
      while not End_Of_File (Dict_File) loop
         ...
         if Length (Read_Word) = Word_Len then
            ...
            if Valid then 
               Cands.Append (To_Bounded_String (To_String (Read_Word)));
            end if;
         end if;
      end loop;
      Close (Dict_File);
   ...
   function Mutate (Word : Bounded_String; Len : Positive; Dict : Word_Vectors.Vector)
            return Word_Vectors.Vector is
      Mutations : Word_Vectors.Vector;
      Poss_Word : Bounded_String;
   begin
   ...
   procedure Ladder (Start_S, Target_S : String) is
      ...
      Word_Rec   : Word_Rec_T;
      Found      : Boolean := False;
      Start, Target, Word : Bounded_String;
   ...
   begin
      ...
      Start      := To_Bounded_String (Start_S);
      Target     := To_Bounded_String (Target_S);
      ...
      if not Found then
         Put_Line (Start_S & " -> " & Target_S & " - No solution found at depth" & MAX_DEPTH'Image);
      else
         Word := Target;
